...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ 
  ...
  // المزامنة التلقائية عند أي تغيير في البيانات
  ...
  const deleteItem = (setter: Function, id: string | number) => {
    if (window.confirm('سيتم حذف العنصر نهائياً من قاعدة بيانات MySQL. تأكيد؟')) {
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center p-6 font-cairo" dir="rtl">
        <div className="w-full max-w-sm bg-[#0a0a0a] border border-white/10 p-12 rounded-[3rem] text-center shadow-2xl">
          ...
          <h2 className="text-xl font-black text-white mb-8 tracking-tighter uppercase">Cloud Admin Login</h2>
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <button type="submit" className="w-full py-5 dynamic-bg text-black font-black rounded-xl hover:scale-105 transition-all text-xs tracking-widest uppercase">دخول النظام</button>
          </form>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-[#050505] text-white flex flex-col lg:flex-row font-cairo" dir="rtl">
      <aside className="w-full lg:w-72 bg-black border-l border-white/5 flex flex-col h-screen sticky top-0 overflow-y-auto z-50">
        <div className="p-8 border-b border-white/5 flex items-center gap-3">
          ...
          <span className="font-black text-[10px] tracking-tighter uppercase">Al-Asimh Cloud CMS</span>
        </div>
        ...
        <button onClick={() => { localStorage.clear(); window.location.reload(); }} className="m-6 p-4 border border-rose-500/20 text-rose-500 rounded-xl text-[9px] font-black uppercase hover:bg-rose-500 hover:text-white transition-all">إعادة تهيئة الربط</button>
      </aside>
      ...
      <main className="flex-grow p-8 lg:p-16 overflow-y-auto">
        <div className="max-w-5xl mx-auto pb-32">
          <header className="mb-16 flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-white/5 pb-10 gap-6">
            <div>
               <h1 className="text-5xl font-black uppercase tracking-tighter mb-2">{activeTab} Console</h1>
               <div className="flex items-center gap-2">
                 <div className={`w-2 h-2 rounded-full ${syncState === 'syncing' ? 'bg-orange-500 animate-pulse' : 'bg-emerald-500'}`}></div>
                 <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{syncState === 'syncing' ? 'Saving to MySQL...' : 'Connected to Server'}</span>
               </div>
            </div>
          ...
          <div className="space-y-12">
            {activeTab === 'dashboard' && (
               <div className="bg-orange-500/10 p-12 rounded-[3rem] border border-orange-500/20 text-right">
                 <h2 className="text-4xl font-black text-orange-500 mb-6 tracking-tighter uppercase">Cloud Native Control</h2>
                 <p className="text-slate-400 leading-relaxed text-sm">أهلاً بك في المحرك المركزي لشركة العاصمة. كل تعديل تقوم به هنا يتم حفظه مباشرة في قاعدة بيانات MySQL السحابية ويتم تحديث الموقع لجميع الزوار فوراً.</p>
                 <div className="mt-10 grid grid-cols-2 sm:grid-cols-4 gap-4">
                    <div className="bg-black/50 p-6 rounded-3xl border border-white/5 text-center">
                       <span className="block text-2xl font-black text-orange-500">{products.length}</span>
                       <span className="text-[9px] text-slate-500 font-bold uppercase">المنتجات</span>
                    </div>
                    <div className="bg-black/50 p-6 rounded-3xl border border-white/5 text-center">
                       <span className="block text-2xl font-black text-orange-500">{galleryItems.length}</span>
                       <span className="text-[9px] text-slate-500 font-bold uppercase">المعرض</span>
                    </div>
                 </div>
               </div>
            )}
            ...
            {activeTab === 'settings' && (
              <div className="space-y-10">
                ...
                <BilingualInput label="Brand Name" valAr={settings.brandName.ar} valEn={settings.brandName.en} setAr={(v:any) => setSettings({...settings, brandName: {...settings.brandName, ar: v}})} setEn={(v:any) => setSettings({...settings, brandName: {...settings.brandName, en: v}})} />
                <BilingualInput label="Tagline" valAr={settings.tagline.ar} valEn={settings.tagline.en} setAr={(v:any) => setSettings({...settings, tagline: {...settings.tagline, ar: v}})} setEn={(v:any) => setSettings({...settings, tagline: {...settings.tagline, en: v}})} />
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                   <div className="space-y-2 text-right"><label className="text-[9px] text-slate-500 font-black">WHATSAPP</label><input className="w-full bg-black border border-white/10 p-4 rounded-xl text-white outline-none focus:border-orange-500" value={settings.whatsapp} onChange={e => setSettings({...settings, whatsapp: e.target.value})} /></div>
                   <div className="space-y-2 text-right"><label className="text-[9px] text-slate-500 font-black">PHONE</label><input className="w-full bg-black border border-white/10 p-4 rounded-xl text-white outline-none focus:border-orange-500" value={settings.phone} onChange={e => setSettings({...settings, phone: e.target.value})} /></div>
                   <div className="space-y-2 text-right"><label className="text-[9px] text-slate-500 font-black">EMAIL</label><input className="w-full bg-black border border-white/10 p-4 rounded-xl text-white outline-none focus:border-orange-500" value={settings.email} onChange={e => setSettings({...settings, email: e.target.value})} /></div>
                </div>
              </div>
            )}
            ...
            {activeTab === 'products' && (
              <div className="space-y-10">
                <button onClick={() => setProducts([{ id: Date.now().toString(), title: { ar: 'منتج جديد', en: 'New Product' }, desc: { ar: 'وصف المنتج هنا..', en: 'Product description here..' }, specs: { ar: [], en: [] }, icon: '🔥', images: [], msg: { ar: 'طلب عرض سعر', en: 'Request quote' } } as any, ...products])} className="w-full py-12 border-2 border-dashed border-white/10 rounded-[3rem] text-slate-500 font-black hover:border-orange-500 hover:text-orange-500 transition-all">+ إضافة منتج تصدير جديد</button>
                {products.map((p, i) => (
                  <div key={p.id} className="bg-[#0a0a0a] p-10 rounded-[3.5rem] border border-white/5 space-y-8 relative group shadow-2xl">
                    <button onClick={() => deleteItem(setProducts, p.id)} className="absolute top-10 left-10 text-rose-500 bg-rose-500/10 px-6 py-2 rounded-xl text-[10px] font-black uppercase">حذف</button>
                    ...
                    <ImageInput label="الصورة الرئيسية" value={p.images[0] || ''} onChange={(v:any) => updateItem(setProducts, i, 'images', [v])} />
                  </div>
                ))}
              </div>
            )}
            ...
            {activeTab === 'blog' && (
              <div className="space-y-10">
                <button onClick={() => setArticles([{ id: Date.now(), title: { ar: 'عنوان المقال', en: 'New Blog Post' }, excerpt: { ar: 'نبذة عن المقال..', en: 'Summary..' }, date: { ar: '2025', en: '2025' }, img: '', category: { ar: 'أخبار', en: 'News' } } as any, ...articles])} className="w-full py-10 border-2 border-dashed border-white/10 rounded-[2.5rem] text-slate-500 font-black">+ نشر خبر أو مقال فني</button>
                {articles.map((a, i) => (
                  <div key={a.id} className="bg-[#0a0a0a] p-10 rounded-[3rem] border border-white/5 space-y-6 relative">
                    <button onClick={() => deleteItem(setArticles, a.id)} className="absolute top-8 left-8 text-rose-500 font-black text-[9px] uppercase">إزالة</button>
                    ...
                    <BilingualInput label="المحتوى" valAr={a.excerpt.ar} valEn={a.excerpt.en} setAr={(v:any) => updateItem(setArticles, i, 'excerpt', v, 'ar')} setEn={(v:any) => updateItem(setArticles, i, 'excerpt', v, 'en')} textarea />
                  </div>
                ))}
              </div>
            )}
                ...
                {stats.map((s, i) => (
                  <div key={s.id} className="bg-black/40 p-10 rounded-[3rem] border border-white/5 space-y-6">
                    <div className="flex gap-4 items-center">
                       ...
                       <input className="flex-grow bg-black border border-white/10 p-4 rounded-xl text-2xl font-black font-sans" value={s.value} onChange={e => updateItem(setStats, i, 'value', e.target.value)} />
                    </div>
                    <BilingualInput label="Label" valAr={s.label.ar} valEn={s.label.en} setAr={(v:any) => updateItem(setStats, i, 'label', v, 'ar')} setEn={(v:any) => updateItem(setStats, i, 'label', v, 'en')} />
                  </div>
                ))}
              </div>
            )}
