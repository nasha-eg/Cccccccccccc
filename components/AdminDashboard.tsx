...
export const AdminDashboard: React.FC<AdminDashboardProps> = (props) => {
  ...
  return (
    <div className="min-h-screen bg-[#020202] text-white flex flex-col lg:flex-row font-cairo overflow-hidden" dir="rtl">
      {/* Overlay for Mobile */}
      {isSidebarOpen && (
        <div 
      ...
      {/* Sidebar Navigation */}
      <aside className={`fixed lg:sticky top-0 right-0 z-[100] h-screen bg-black border-l border-white/5 flex flex-col p-8 transition-all duration-500 ease-in-out ${isSidebarOpen ? 'w-[300px] sm:w-[350px] translate-x-0' : 'w-0 -translate-x-full lg:translate-x-0 lg:w-0 lg:p-0 lg:overflow-hidden'}`}>
        <div className="mb-12 flex items-center gap-5 p-4 bg-zinc-900/30 rounded-3xl border border-white/5 whitespace-nowrap">
           <div className="w-14 h-14 dynamic-bg rounded-2xl flex-shrink-0 flex items-center justify-center text-black font-black text-2xl shadow-lg">A</div>
        ...
        <nav className="flex-grow space-y-2 overflow-y-auto custom-scrollbar pr-2 whitespace-nowrap">
          {menuItems.map(item => (
            <button 
              key={item.id} 
              onClick={() => {
              ...
              className={`w-full flex items-center gap-5 px-8 py-5 rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all ${activeTab === item.id ? 'bg-orange-500 text-black shadow-2xl scale-[1.03]' : 'text-zinc-500 hover:bg-white/5 hover:text-white'}`}
            >
        ...
        <div className="mt-10 pt-8 border-t border-white/5 space-y-5 whitespace-nowrap">
      ...
      {/* Main Content Area */}
      <main className="flex-grow flex flex-col bg-[#060606] h-screen overflow-hidden">
        {/* Admin Bar Header */}
        <div className="h-20 border-b border-white/5 flex items-center justify-between px-8 bg-[#020202]">
           <button 
             onClick={() => setIsSidebarOpen(!isSidebarOpen)}
             className="w-12 h-12 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center hover:bg-orange-500 hover:text-black transition-all group"
           >
             <svg className={`w-6 h-6 transition-transform duration-500 ${isSidebarOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
           ...
           <div className="flex items-center gap-6">
             <div className="hidden sm:flex flex-col text-left items-end">
               <span className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">مرحباً، المشرف</span>
               <span className="text-xs font-black text-white">إدارة شركة العاصمة</span>
             </div>
             <a href="/" target="_blank" className="w-10 h-10 rounded-lg bg-orange-500/10 border border-orange-500/20 flex items-center justify-center text-orange-500 hover:bg-orange-500 hover:text-black transition-all">
               <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
             </a>
           </div>
        </div>

        <div className="flex-grow overflow-y-auto custom-scrollbar p-8 lg:p-16">
